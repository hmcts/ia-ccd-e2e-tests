Feature: Notice of Change (common component)
  ...
  @notice-of-change @noc-incorrect-answers-same-org @noc-incorrect-firstname-same-org @RIA-3745
  Scenario: NoC request as a user from the same organisation (incorrect firstname anwser)
    ...
    When I click the `Notice of change` link
  ...
  Scenario: NoC request as a user from a different organisation (correct answers)
    ...
    Given I am signed in as a `Legal Org User Rep A`
    When I get the most recent case id after successful notice of change
    Then I confirm the notice of change case is no longer available
