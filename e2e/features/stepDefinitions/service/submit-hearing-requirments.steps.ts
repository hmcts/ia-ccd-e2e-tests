...
Then(/^I should (see|not see) the hearing requirements (yes|no) path for (hoUsers|nonHoUsers)$/, async function (seeOrNotSee, yesOrNo, hoUsers) {

    const isDisplayed = (seeOrNotSee === 'see');
    const isYesPath = (yesOrNo === 'yes');
    const isHomeOffice = (hoUsers === 'hoUsers');
