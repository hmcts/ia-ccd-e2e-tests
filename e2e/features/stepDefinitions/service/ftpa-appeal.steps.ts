...
Then(/^I verify the (presence|absence) of respondent FTPA documents under (Documents|FTPA) tab$/, async function (seeOrNotSee, documentOrFtpa) {
    ...
    if (isDisplayed) {
        if (isFtpa) {
            expect(await ccdPage.headingContains('FTPA', isDisplayed)).to.equal(isDisplayed);
            expect(await ccdPage.contentContains('Respondent: application for permission to appeal', Wait.instant)).to.equal(true);
            ...
            expect(await ccdPage.headingContains('Documents', isDisplayed)).to.equal(isDisplayed);
            expect(await ccdPage.isFieldValueDisplayed('Document', 'FTPARespondentGrounds.pdf', true, 1, 'FTPA Respondent documents')).to.equal(true);
            expect(await ccdPage.isFieldValueDisplayed('Description', 'This is the FTPA Respondent grounds', true, 1, 'FTPA Respondent documents')).to.equal(true);
            expect(await ccdPage.isFieldValueDisplayed('Date uploaded', '{$TODAY|D MMM YYYY}', true, 1, 'FTPA Respondent documents')).to.equal(true);
            expect(await ccdPage.isFieldValueDisplayed('Description', 'This is the FTPA Respondent evidence', true, 1, 'FTPA Respondent documents', 1)).to.equal(true);
            expect(await ccdPage.isFieldValueDisplayed('Document', 'FTPARespondentEvidence.pdf', true, 1, 'FTPA Respondent documents', 1)).to.equal(true);
            expect(await ccdPage.isFieldValueDisplayed('Date uploaded', '{$TODAY|D MMM YYYY}', true, 1, 'FTPA Respondent documents', 1)).to.equal(true);
        }
    } else {
        ...
        if (isFtpa) {
            expect(await ccdPage.contentContains('Respondent: application for permission to appeal', Wait.instant)).to.equal(false);
            expect(await ccdPage.contentContains('Grounds of the application', Wait.instant)).to.equal(false);
            expect(await ccdPage.contentContains('Supporting evidence', Wait.instant)).to.equal(false);
        } else {
            expect(await ccdPage.contentContains('FTPA Respondent documents', Wait.instant)).to.equal(false);
        }
    }
});
