...
Then(
  ...
  async function (seeOrNotSee, yesOrNo) {
    ...
    if (isDisplayed) {
      if (isYesPath) {
        expect(
          await ccdPage.headingContains("Requests for additional adjustments")
        ).to.equal(true);
        expect(
...
Then(
  ...
  async function (seeOrNotSee, yesOrNo) {
    ...
    expect(
      await ccdPage.headingContains(
        "Agreed additional adjustments",
        isDisplayed
      )
    ...
    if (isDisplayed) {
      if (isYesPath) {
        ...
        expect(
          await ccdPage.contentContains("Remote hearing", Wait.instant)
        ).to.equal(true);
        expect(
          await ccdPage.contentContains(
            "Adjustments to accommodate vulnerabilities",
        ...
        expect(
          await ccdPage.contentContains("Multimedia equipment", Wait.instant)
        ).to.equal(false);
        expect(
          await ccdPage.contentContains("Single-sex court", Wait.instant)
        ).to.equal(false);
        expect(
          await ccdPage.contentContains("In camera court", Wait.instant)
        ).to.equal(false);
        expect(
          await ccdPage.contentContains("Other adjustments", Wait.instant)
        ).to.equal(false);
      }
    } else {
      expect(
