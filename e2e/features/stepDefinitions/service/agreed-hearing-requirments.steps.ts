...
Then(/^I should (see|not see) the requests for additional adjustments (yes|no) path$/, async function (seeOrNotSee, yesOrNo) {
  ...
  if (isDisplayed) {
    if (isYesPath) {
      ...
      expect(await ccdPage.contentContains("You should provide the equipment to play this evidence. If this is not possible, explain why and what equipment you'll need to play it. (Optional)", Wait.instant)).to.equal(true);
    ...
    expect(await ccdPage.contentContains("You should provide the equipment to play this evidence. If this is not possible, explain why and what equipment you'll need to play it. (Optional)", Wait.instant)).to.equal(false);
