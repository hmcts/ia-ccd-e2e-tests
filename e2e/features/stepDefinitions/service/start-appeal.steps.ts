...
Given("I save my initial appeal with address", async() => {
  if (isfeePaymentEnabled) {
    await startAppealFlow.saveInitialAppealWithFee(
      ...
      "hearing fee",
      true,
      `Airborne Studios, 52 Newton Street, Manchester`,
      `M1 1ED`
    );
  } else {
  ...
  async appellantInUk => {
    if (isOutOfCountryEnabled) {
      if (isfeePaymentEnabled) {
        ...
        await startAppealFlow.saveInitialNonPaymentAppealOutOfCountry(
          true,
          "PA",
          appellantInUk
        );
  ...
  async(lateAppeal, decisionType) => {
    if (isOutOfCountryEnabled) {
      if (isfeePaymentEnabled) {
        ...
        await startAppealFlow.saveInitialNonPaymentAppealOutOfCountryWithDecision(
          true,
          "PA",
          "No",
          decisionType,
  ...
  async(
    ...
    if (isOutOfCountryEnabled) {
      if (isfeePaymentEnabled) {
        ...
        await startAppealFlow.saveInitialNonPaymentAppealOutOfCountryWithSponsor(
          true,
          givenName,
          familyName,
          contactPreference,
...
Given(
  /^I save my initial (aip|lr) internal ([A-Za-z]+) nondetained appeal for nonPayment (with|without) hearing$/,
  async function (internalJourneyType, appealType, hearingOption) {
    await startAppealFlow.saveInitialAipInternalNonPaymentNonDetainedAppeal(
      true,
      appealType,
      hearingOption,
      internalJourneyType
