...
Given('I create a new bail application', async function () {
  ...
  if (iaConfig.CcdWebUrl.includes('aat') || iaConfig.CcdWebUrl.includes('pr')) {
    await ccdFormPage.setFieldValue('Case type', 'Bail* RIA-8913-natio');
  } else if (iaConfig.CcdWebUrl.includes('demo')) {
...
Then(/^I should (see|not see) the `?(first|second|third|)`?\s?(?:answer|field) without a label$/, async function (seeOrNotSee, instanceNumber) {
  expect(await ccdPage.isFieldDisplayed('', instanceNumber)).to.equal(seeOrNotSee === 'see');
});

Then(/^I should (see|not see) the `?(first|second|third|)`?\s?`?([^`]+)`? (?:answer|field)$/, async function (seeOrNotSee, instanceNumber, fieldLabel) {
  expect(await ccdPage.isFieldDisplayed(fieldLabel, instanceNumber)).to.equal(seeOrNotSee === 'see');
});

Then(/^the `?(first|second|third|)`?\s?`?([^`]+)`? (?:answer|field) should be empty$/, async function (instanceNumber, fieldLabel) {
  expect(await ccdPage.isFieldValueDisplayed(fieldLabel, '', true, instanceNumber)).to.equal(true);
});

Then(/^the `?(first|second|third|)`?\s?`?([^`]+)`? (?:answer|field) should be (\d+) characters long$/, async function (instanceNumber, fieldLabel, fieldValueSize) {
  expect(await ccdPage.isFieldValueCorrectLength(fieldLabel, fieldValueSize, instanceNumber)).to.equal(true);
});

Then(/^the `?(first|second|third|)`?\s?(?:answer|field) without a label should be empty$/, async function (instanceNumber) {
  expect(await ccdPage.isFieldValueDisplayed('', '', true, instanceNumber)).to.equal(true);
});

Then(/^I should see `?([^`]+)`? (in|for) the `?(first|second|third|)`?\s?(?:answer|field) without a label$/, async function (fieldMatch, inOrFor, instanceNumber) {
...
Then(/^within the `?(first|second|third|)`?\s?`?([^`]+)`? fieldset, I should see `?([^`]+)`? (in|for) the (?:answer|field) without a label$/, async function (instanceNumber, fieldsetLabel, fieldMatch, inOrFor) {
  const isExactMatch = inOrFor === 'for';

  expect(await ccdPage.isFieldValueDisplayed('', fieldMatch, isExactMatch, instanceNumber, fieldsetLabel)).to.equal(true);
});

Then(/^within the `?(first|second|third|)`?\s?`?([^`]+)`? fieldset, I should see `?([^`]+)`? (in|for) the `?([^`]+)`? (?:answer|field)$/, async function (instanceNumber, fieldsetLabel, fieldMatch, inOrFor, fieldLabel) {
  const isExactMatch = inOrFor === 'for';

  expect(await ccdPage.isFieldValueDisplayed(fieldLabel, fieldMatch, isExactMatch, instanceNumber, fieldsetLabel)).to.equal(true);
});

Then(/^within the `?(first|second|third|)`?\s?`?([^`]+)`? collection's `?([^\s`]+)`? item, I should see `?([^`]+)`? (in|for) the (?:answer|field) without a label$/, async function (instanceNumber, collectionLabel, collectionItemNumber, fieldMatch, inOrFor) {
  const isExactMatch = inOrFor === 'for';

  expect(await ccdPage.isFieldValueDisplayed('', fieldMatch, isExactMatch, instanceNumber, collectionLabel, collectionItemNumber)).to.equal(true);
});

Then(/^within the `?(first|second|third|)`?\s?`?([^`]+)`? collection's `?([^\s`]+)`? item, I should see `?([^`]+)`? (in|for) the `?([^`]+)`? (?:answer|field)$/, async function (instanceNumber, collectionLabel, collectionItemNumber, fieldMatch, inOrFor, fieldLabel) {
  const isExactMatch = inOrFor === 'for';

  expect(await ccdPage.isFieldValueDisplayed(fieldLabel, fieldMatch, isExactMatch, instanceNumber, collectionLabel, collectionItemNumber)).to.equal(true);
});
...
Then(/^I Add the interpreter details$/, async function () {
  ...
  await ccdFormPage.setFieldValue('Note (Optional)', 'Adding new spoken or sign language intrpreter');
...
Then(/^I update interpreter booking status$/, async function () {
  ...
  await ccdFormPage.setFieldValue('Harri Pugh - Applicant - British Sign Language (BSL)', 'Booked');
  await browser.sleep(3000);
  await ccdFormPage.setFieldValue('John Smith - FCS - Telugu', 'Booked');
  await ccdFormPage.setFieldValue('John Smith - FCS - British Sign Language (BSL)', 'Requested');
  await ccdFormPage.setFieldValue('Jane Doe - FCS - Hindi', 'Requested');
  await ccdFormPage.setFieldValue('Jane Doe - FCS - British Sign Language (BSL)', 'Cancelled');
  await ccdFormPage.click('Continue');
  await browser.sleep(3000);
  await ccdFormPage.click('Update');
});
