Feature: Submit appeal application failed payment (PA, EA and HU appeal types)
  ...
  @failed-payment-pa-appeal-type @RIA-3082
  Scenario: Submit an appeal application with a failed payment (PA appeal type)
    ...
    Then I select `Pay and submit the appeal now` for the `When will you pay?` field
    And I click the `Continue` button
    ...
    And I should see `Pay and submit the appeal now` for the `When will you pay?` answer
    ...
    And I type `PBA1234567` for the `Enter your Payment by Account reference number` field
    ...
    And I should see the text `PBA1234567`
    ...
    And I should see `Failed` for the `Payment status` field


  @failed-payment-ea-appeal-type @RIA-3082
  Scenario: Submit an appeal application with a failed payment (EA appeal type)
  ...
  @failed-payment-hu-appeal-type @RIA-3082
  Scenario: Submit an appeal application with a failed payment (HU appeal type)
