Feature: Other Appeal Numbers are validated
  ...
  Background:
    ...
    And I am on the `Other appeals` page
    And I select `Yes` for the `Has the appellant appealed against any other UK immigration decision?` field
    And I click the `Continue` button

  @start-appeal @alternate @RIA-635 @migrate-to-unit-tests
  Scenario: Invalid other appeal references

    Given I am on the `Provide the appeal number of any other appeals` page
    ...
    And I click the `Continue` button
    Then I should see an error saying `The data entered is not valid for Field`
  ...
  Scenario: Valid appeal references
    ...
    And I am on the `Provide the appeal number of any other appeals` page
