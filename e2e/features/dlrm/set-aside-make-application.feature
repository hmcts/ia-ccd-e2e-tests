Feature: Set Aside Make an Appplication for rule 31 and 32
    ...
    @set-aside-dlrm @RIA-8228 @dlrm-works
    Scenario Outline: Set Aside Make an Appplication for rule 31 and 32
        ...
        And within the `Application` collection's first item, I should see `Set aside a decision` in the `Type of application` field
        And within the `Application` collection's first item, I should see `Test reason for  Set aside a decision` in the `Application details` field
