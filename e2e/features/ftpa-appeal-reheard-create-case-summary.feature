Feature: Admin Officer lists reheard case - FTPA reheard decision (resident judge)
  ...
  @Create-case-summary-in-reheard @RIA-3129
  Scenario: Case officer create and upload case summary in reheard
    ...
    When I switch to be a `Case Officer`
    Then I select the `Create case summary` Next step
    And I am on the `Create case summary` page
    ...
    And within the `Reheard hearing documents` collection's first item, I should see `CaseSummary.pdf` in the `Document` field
    And within the `Reheard hearing documents` collection's first item, I should see `This is the case summary` in the `Description` field
