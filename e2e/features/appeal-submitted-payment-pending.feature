Feature: Submit appeal application moves to Payment Pending state for pay later (EA and HU appeal types)
  ...
  @payment-pending-ea-appeal-type @RIA-3278
  Scenario: Submit an appeal application (EA appeal type) and move to payment pending
    ...
    And I should see `Submit the appeal now and pay within 14 days` for the `When will you pay?` answer
    ...
    And I should see the text `Your appeal has been submitted`
    ...
    And I should see `Payment due` for the `Payment status` field

    When I click the `Overview` tab
    Then I should only see the `legalRep_appealSubmitted` case progress image
    And I should see the text `Do this next`
    And I should see the text `You have submitted your appeal. A case officer will now review your appeal. You don't need to do anything else right now.`
  ...
  @payment-pending-hu-appeal-type @RIA-3278
  Scenario: Submit an appeal application (HU appeal type) and move to payment pending
