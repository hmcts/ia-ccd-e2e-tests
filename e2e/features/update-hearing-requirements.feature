Feature: Update hearing requirements
  ...
  @update-hearing-requirements-yes-to-no @RIA-2031
  Scenario: Update hearing requirements to No answers
    ...
    And within the `Hearing requirements and requests` collection's second item, I should see `-Gonzlez-hearing-requirements.PDF` in the `Document` field
    And within the `Hearing requirements and requests` collection's second item, I should see `{$TODAY|D MMM YYYY}` for the `Date uploaded` field
  ...
  @update-hearing-requirements-no-to-yes @RIA-2031
