Feature: Review time extension
  ...
  Scenario: A Case officer can review an appellants time extension approved
    Given An appellant has submitted an appeal
    And I am signed in as a `Case Officer`
    And I am viewing the appellant's case
    And I request respondent evidence
    ...
    When I approve the time extension
    Then I am on the `Check your answers` page
    And I should see `Application granted` in the `Decision` field
    And I should see `decision reason` in the `Reason for decision` field
    And I should see `${TODAY}` in the `Due Date` field
