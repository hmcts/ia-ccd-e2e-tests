Feature: SnL Flow Update hearing requirements
  ...
  Scenario: SnL flow Review hearing requirements with 'Agreed' options selected
   ...
   And I select `No` for the `Does the appellant need an in camera court?` field
    ...
    And I should see `Telugu` in the fifth `Spoken language` field

    # And I should see `Akan` in the sixth `Spoken language` field

    # And I should see `Lipspeaker` in the sixth `Sign language` field

    # And I should see `Telugu` in the seventh `Spoken language` field

    # And I should see `Makaton` in the eighth `Sign language` field
