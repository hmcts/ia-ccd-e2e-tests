...
export class CcdWriteMultiSelectListFieldFinder implements FieldFinder {
  ...
  public async findByLabel(container, instanceNumber: number, fieldLabel: string) {
    ...
    if ((await fieldContainer.isPresent()) && (await fieldContainer.$$('ccd-write-multi-select-list-field').isPresent())) {
      return new CcdWriteMultiSelectListField(fieldContainer, fieldLabel);
    }
  }

  public async findHavingEmptyLabel(container, instanceNumber: number) {
    const fieldContainer = container.all(by.xpath('.//ccd-field-write[.//ccd-write-multi-select-list-field]')).get(instanceNumber - 1);

    if (await fieldContainer.isPresent()) {
      return new CcdWriteMultiSelectListField(fieldContainer, '');
    }
  }
}
