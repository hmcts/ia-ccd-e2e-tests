...
export class CcdWriteTelFieldFinder implements FieldFinder {
  ...
  public async findByLabel(container, instanceNumber: number, fieldLabel: string) {
    ...
    if ((await fieldContainer.isPresent()) && (await fieldContainer.$$('ccd-write-phone-uk-field').isPresent())) {
      return new CcdWriteTelField(fieldContainer, fieldLabel);
    }
  }
  ...
  public async findHavingEmptyLabel(container, instanceNumber: number) {
    ...
    if (await fieldContainer.isPresent()) {
      return new CcdWriteTelField(fieldContainer, '');
    }
  }
}
