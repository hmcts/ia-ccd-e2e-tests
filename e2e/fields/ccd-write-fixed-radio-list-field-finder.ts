...
export class CcdWriteFixedRadioListFieldFinder implements FieldFinder {
  ...
  public async findHavingEmptyLabel(container, instanceNumber: number) {
    const fieldContainer = container.all(by.xpath('.//ccd-field-write[.//ccd-write-fixed-radio-list-field]')).get(instanceNumber - 1);

    if (await fieldContainer.isPresent()) {
      return new CcdWriteFixedRadioListField(fieldContainer, '');
    }
  }
}
