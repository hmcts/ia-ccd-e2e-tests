...
export class CcdWriteTextAreaFieldFinder implements FieldFinder {
  ...
  public async findByLabel(container, instanceNumber: number, fieldLabel: string) {
    ...
    if ((await fieldContainer.isPresent()) && (await fieldContainer.$$('ccd-write-text-area-field').isPresent())) {
      return new CcdWriteTextAreaField(fieldContainer, fieldLabel);
    }
  }

  public async findHavingEmptyLabel(container, instanceNumber: number) {
    const fieldContainer = container.all(by.xpath('.//ccd-field-write[.//ccd-write-text-area-field]')).get(instanceNumber - 1);

    if (await fieldContainer.isPresent()) {
      return new CcdWriteTextAreaField(fieldContainer, '');
    }
  }
}
