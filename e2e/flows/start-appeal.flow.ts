...
export class StartAppealFlow {
  ...
  async completeInternalScreeningQuestions(clickContinue = false) {
    ...
    await this.ccdFormPage.click('Continue');
    await this.ccdFormPage.setFieldValue('Is the appellant in the UK?', 'Yes');
    await this.ccdFormPage.runAccessbility();
    if (clickContinue) {
      await this.ccdFormPage.click('Continue');
    }
  ...
  async completeInternalHomeOfficeReference(clickContinue = false, ooc = false, homeOfficeReferenceNumber = '') {
    ...
    if (ooc) {
      await this.ccdFormPage.setFieldValue('What date was the Home Office decision letter sent?', '{$TODAY-2}');
    } else {
  ...
  async completeUploadNoticeDecision(clickContinue = false) {
    ...
    await this.ccdFormPage.setFieldValue('Document', '{@Evidence1.pdf}', 'document', 'first', 'Notice of Decision', 'first');
    await this.ccdFormPage.setFieldValue('Describe the document', 'This is the notice of decision', 'text area', 'first', 'Notice of Decision', 'first');
  ...
  async completeInternalUploadNoticeDecisionNoUpload(clickContinue = false) {
    await this.ccdFormPage.runAccessbility();
    await browser.sleep(1000);
    await this.ccdFormPage.click('Add new');
    let fileDetector = WebDriver.fileDetector;
  ...
  async completeBasicDetails(clickContinue = false, isInternalCase = false) {
    await this.ccdFormPage.runAccessbility();
    if( isInternalCase == false) {
  ...
  async completeClientAddress(clickContinue = false, hasFixedAddress = false, address = '', postcode = '', isInternalCase = false) {
    let fieldLabel = 'Does the appellant have a fixed address?';
    if( isInternalCase ) {
      fieldLabel = 'Does the appellant have a postal address?';
    }
    await this.ccdFormPage.runAccessbility();
    if (!hasFixedAddress) {
      await this.ccdFormPage.setFieldValue(fieldLabel, 'No');
    } else {
      await this.ccdFormPage.setFieldValue(fieldLabel, 'Yes');
  ...
  async completeGivenAppealType(clickContinue = false, appealType) {
    ...
    if (appealType === 'EU') {
      await this.ccdFormPage.setFieldValue('Type of appeal', 'Refusal of application under the EU Settlement Scheme');
    }
    if (clickContinue) {
      await this.ccdFormPage.click('Continue');
    }
  ...
  async completeDeportationOrder(clickContinue = false) {
    await this.ccdFormPage.runAccessbility();
    await browser.sleep(2000);
    await this.ccdFormPage.setFieldValue('Has a deportation order been made against the appellant?', 'Yes');
  ...
  async completeOtherAppeals(clickContinue = false, isInternalCase = false) {
    let fieldLabel = isInternalCase ? 'Has the appellant appealed against any other UK immigration decision?' : 'Other appeals';
    await this.ccdFormPage.runAccessbility();
    await this.ccdFormPage.setFieldValue( fieldLabel, 'No');
  ...
  async completeLegalRepresentativeDetails(clickContinue = false) {
    ...
    // await this.ccdFormPage.setFieldValue('Contact number', '01212121212');
  ...
  async saveInitialInternalAppealWithoutRemission(clickContinue = false, appealType = '', feeType = '', paymentChoice = '', hasFixedAddress = false, address = '', postcode = '') {
    ...
    await this.completeDeportationOrder(true);
    // await this.completeNewMatters(true);
    await this.completeOtherAppeals(true, true);
    //await this.completeLegalRepresentativeDetails(true);
    await this.completeGivenFee(true, feeType);
    await this.completeRemissionDetails(true, 'no remission', true);
  ...
  async completeOutOfTimeClientDetails(clickContinue = false, hasFixedAddress = false, address = '', postcode = '') {
    ...
    await this.completeBasicDetails(true);
    await this.completeNationality(true);
    await this.completeClientAddress(true, hasFixedAddress, address, postcode, true);
    await this.completeContactPreference(true);
  }
  ...
  async completeRemissionDetails(clickContinue = false, remissionOption = '', isInternalCase = false) {
    let prefixValue = isInternalCase ?  'The appellant' : 'My client';
    await this.ccdFormPage.runAccessbility();
    if (remissionOption === 'no remission') {
      await this.ccdFormPage.setFieldValue('Choose one of the following statements', `${prefixValue} is not eligible for a fee remission`);
    }
    if (remissionOption === 'a remission') {
      await this.ccdFormPage.setFieldValue('Choose one of the following statements', `${prefixValue} has a remission, e.g. Asylum support, Legal Aid, Home Office waiver, Section 17/20`);
      await this.ccdFormPage.click('Continue');
      await this.ccdFormPage.setFieldValue('Choose one of the following statements', `${prefixValue} receives Legal Aid`);
      await this.ccdFormPage.click('Continue');
      await this.ccdFormPage.setFieldValue('Legal Aid account number', '1A234B');
    }
    if (remissionOption === 'help with fees') {
      await this.ccdFormPage.setFieldValue('Choose one of the following statements', `${prefixValue} has a Help with Fees reference number`);
    }
  ...
  async completeDecisionType(clickContinue = false, decisionOption = '') {
    if (decisionOption === 'refusalOfHumanRights') {
      await this.ccdFormPage.setFieldValue('What type of decision are you appealing?', 'A decision either 1) to refuse a human rights claim made following an application for entry clearance or 2) to refuse a permit to enter the UK under the Immigration (European Economic Area) Regulation 2016');
    } else if (decisionOption === 'refusalOfProtection') {
      await this.ccdFormPage.setFieldValue('What type of decision are you appealing?', 'A decision to refuse a protection or human rights claim where your client may only apply after leaving the UK');
    ...
    if (decisionOption === 'refusePermitOfEUSS') {
      await this.ccdFormPage.setFieldValue('What type of decision are you appealing?', 'A decision to refuse a permit to enter the UK under the immigration rules and/or the EU Settlement Scheme.');
    }
    if (clickContinue) {
      await this.ccdFormPage.click('Continue');
    }
