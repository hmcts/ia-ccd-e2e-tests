...
export class ForceACaseProgressionFlow {
    ...
    async forceRequestToCaseBuilding(clickContinue = false) {
        ...
        await this.ccdFormPage.setFieldValue('Reasons to force the case progression', 'some reasons to force the case progression');
        await this.ccdFormPage.click('Continue');
    ...
    async forceRequestToCaseUnderReview(clickContinue = false) {
        ...
        await this.ccdFormPage.setFieldValue('Reasons to force the case progression', 'some reasons to force the case progression - case under review');
        await this.ccdFormPage.click('Continue');
    ...
    async forceCaseToHearingRequirments(clickContinue = false) {
        ...
        await this.ccdFormPage.setFieldValue('Reasons to force the case progression', 'some reasons to force the case progression - case hear req');
        await this.ccdFormPage.click('Continue');
        await this.ccdFormPage.click('Submit');
        if (clickContinue) {
            await this.ccdFormPage.click('Close and Return to case details');
